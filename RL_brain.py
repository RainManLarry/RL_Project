...
class DeepQNetwork:
    def __init__(
        ...
        # total learning step #用这个记录学习了多少步，可以让self.epsilon根据这个步数来不断提高
    ...
    def _build_net(self):
        # ------------------ build evaluate_net ------------------
        self.s = tf.placeholder(tf.float32, [None, self.n_features], name='s')  # input 输入当前状态，作为NN的输入
        self.q_target = tf.placeholder(tf.float32, [None, self.n_actions], name='Q_target')  # for calculating loss 输入Q现实为了后面误差计算反向传递
